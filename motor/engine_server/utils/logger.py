...
from motor.engine_server.constants.constants import (LOG_DEFAULT_FILE_PATH, LOG_MAX_LINE_LENGTH, LOG_DATE_FORMAT,
                                                     ...
                                                     LOG_BAK_PRIVILEGE,
                                                     ENGINE_SERVER_LOG_STDOUT, ENGINE_SERVER_LOG_PATH,
                                                     ENGINE_SERVER_FILE_LOG_LEVEL,
                                                     ENGINE_SERVER_STD_LOG_LEVEL)
from motor.engine_server.utils.validators import FileValidator, DirectoryValidator
...
class LogConfig:
    ...
    def build_log_path(self):
        log_path = os.getenv(ENGINE_SERVER_LOG_PATH)
        if log_path is None:
        ...
        if Path(log_path).is_symlink():
            raise Exception("engine server log path should not be a soft link")
        if not DirectoryValidator(log_path).check_directory_permissions(LOG_DIR_PRIVILEGE).check().is_valid():
            raise Exception(f"engine server log path permission is not {oct(LOG_DIR_PRIVILEGE)}")
    ...
    def build_loglevel(self):
        file_log_level = os.getenv(ENGINE_SERVER_FILE_LOG_LEVEL)
        if file_log_level is not None:
            self.file_log_level = file_log_level
        std_log_level = os.getenv(ENGINE_SERVER_STD_LOG_LEVEL)
        if std_log_level is not None:
            self.std_log_level = std_log_level

    def build_log_stdout(self):
        log_stdout = os.getenv(ENGINE_SERVER_LOG_STDOUT)
...
def _get_logger(file_name: str) -> logging.Logger:
...
run_log = _get_logger("engine_server")
