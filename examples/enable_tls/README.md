...
通过`gen_ca_openssl.sh`生成ca证书，需要手动执行

通过`gen_cert_openssl.sh`生成服务端证书，在`boot.sh`中执行

## 2. 修改`boot.sh`

### 2.1 新增`apply_openssl_gen_cert`函数
...
apply_openssl_gen_cert() {
    ...
    local gen_cert_script="${script_dir}/../examples/https/openssl_gen_cert.sh"
...
CA_PATH="/usr/local/Ascend/pyMotor/conf/security/ca"
